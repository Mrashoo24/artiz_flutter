...
class SearchProductListWidget extends StatelessWidget {
  final List<EProvider> services;

  SearchProductListWidget({Key key, List<EProvider> this.services}) : super(key: key);

  @override
  Widget build(BuildContext context) {

    print('gotSearchService = ${services.length}');
    ...
    return Obx(() {
      if (this.services.isEmpty) {
        ...
        return ListView.builder(
          ...
          itemBuilder: ((_, index) {
            var _service = services.elementAt(index);
            return ProvidersListItemWidget(service: _service);
          }),
        );
      }
    });
