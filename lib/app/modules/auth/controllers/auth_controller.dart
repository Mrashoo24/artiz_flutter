  ...
  void login() async {
    ...
    if (loginFormKey.currentState.validate()) {
      ...
      try {
       ...
       var result = await _userRepository.signInWithEmailAndPassword(currentUser.value.email, currentUser.value.apiToken);
        if( result ) {await Get.find<RootController>().changePageOutRoot(0);
  ...
  void register() async {
    ...
    if (registerFormKey.currentState.validate()) {
      ...
      try {
        ...
        bool result =  await _userRepository.signUpWithEmailAndPassword(currentUser.value.email, currentUser.value.apiToken);
          if( result ) {await Get.find<RootController>().changePageOutRoot(0);
          await Get.find<RootController>().changePage(0);}
        }
      } catch (e) {
