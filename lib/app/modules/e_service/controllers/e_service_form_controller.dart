...
import 'e_service_controller.dart';
  ...
  * */
  ...
  void createEServiceForm({bool createOptions = false}) async {
    ...
    if (eServiceForm.currentState.validate()) {
      try {
        ...
        eService.value.ph_no = Get.find<AuthService>().user.value.phoneNumber;
  ...
  void updateEServiceForm() async {
    ...
    if (eServiceForm.currentState.validate()) {
      try {
        ...
        Get.find<EServiceController>().refreshEService();
        Get.find<EServiceController>().eService.value = eService.value;

        Get.offAndToNamed(Routes.E_SERVICE, arguments: {'eService': eService.value, 'heroTag': 'e_service_update_form'});
      } catch (e) {
        Get.showSnackbar(Ui.ErrorSnackBar(message: e.toString()));
      } finally {}
    } else {
  ...
  void deleteEService() async {
    try {
      ...
      Get.offAndToNamed(Routes.E_SERVICE);
